(function () {
  ...
  function boot(cfg) {
    ...
    shadow.innerHTML = '<div class="sc-root">' +
      ...
      '<div class="sc-panel" id="sc-panel">' +
        ...
        '<div class="sc-messages" id="sc-messages">' +
          '<div class="sc-msg sc-bot sc-fadein">' + esc(welcome) + '</div>' +
        '</div>' +
        '<div class="sc-footer">' +
    ...
    function markUnread() {
      if (!isOpen) { unread = true; dot.classList.add("sc-show"); }
    }
    ...
    newBtn.onclick = function () {
      ...
      sid = "sc_" + Math.random().toString(36).slice(2) + Date.now().toString(36);
      sessionStorage.setItem("_sc_sid", sid);
      msgList.innerHTML = '<div class="sc-msg sc-bot sc-fadein">' + esc(welcome) + '</div>';
    };
    ...
    function addMsg(text, type, asText) {
      var div = document.createElement("div");
      div.className = "sc-msg sc-" + type + " sc-fadein";
      if (text && asText) div.textContent = text;
      else if (text) div.innerHTML = renderMd(text);
  ...
  function buildCSS(color, pos) {
    ...
    return [
      ...
      ".sc-header-btns { display:flex; gap:6px; align-items:center; }",
